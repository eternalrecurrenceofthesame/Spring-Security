# 권한 부여 구성: 액세스 제한
```
권한 부여(Authorization) 는 식별(Authentication) 된 클라이언트가 요청된 리소스에 액세스할 권한이 있는지 시스템이 결정하는 프로세스다.
인증(Authentication) 필터를 통해 사용자를 식별하면 사용자 세부 정보가 보안 컨텍스트에 저장되고, 요청이 권한 부여 필터로 위임된다. 171p

권한 부여 필터에서 요청을 허용할지 결정하고, 권한이 허용되면 요청 컨트롤러로 전달된다.

식별 == 인증 == 로그인 == Authentication 같은 의미! 

인증(Authentication) 필터 // 권한 부여(Authorization) 필터 는 서로 다른것! 
```
#### + OAuth 2 프레임워크 구조에서의 인증(Authentication) 인가(Authorization)
```
스프링 시큐리티로 OAuth 2 프레임워크 승인 코드 그랜트 유형을 구현할 때는 인증과 인가 과정이 분리되어서 일어난다 사용자가 리소스
서버에 접근하면 AuthorizationServer 는 ClientServer(웹 애플리케이션) 을 Authentication 화면으로

리다이렉트하고 사용자는 로그인한다. 로그인에 성공하면 ClientServer 는 승인코드와 상태값을 요청하고 AuthorizationServer 는
승인코드와 상태값을 넘겨주는데 ClientServer 는 이 데이터를 사용해서 리소스에 접근할 수 있는 액세스 토큰을 얻을 수 있다.

액세스 토큰을 얻은 클라이언트 애플리케이션은 리소스 서버를 호출할 수 있다. 앞서 설명한 시큐리티에서 권한 부여를 하는

과정과 다른점은 Client, Authorization, Resource 세 개가 분리되어 있으며 사용자의 자격 증명(Authentication) 을 서로
공유하지 않는다는 점이다.

개념적으로 헷갈릴 수있으므로 OAuth 2 프레임워크 구조에서는 인증과 인가 과정을 서로 분리하고 다른 용어를 사용한다. 
```
## 사용자 권한을 기준으로 엔드포인트에 접근 제한
```
애플리케이션의 권한 부여 필터는 보안 컨텍스트에 저장된 인증된 사용자의 세부 정보(Authentication) GrantedAuthority 컬렉션의
권한에 따라 권한 부여를 수행한다.

권한을 기준으로 접근을 제한하지 않으면 사용자가 애플리케이션의 모든 엔드포인트를 호출할 수 있다. 권한은 사용자가 수 행할 수
있는 작업을 의미한다.
```
```
...
참고로 권한은 대문자 또는 소문자로 지정할 수 있다. 어떤 것을 사용하더라도 애플리케이션에서 일관성을 유지한다면 상관없다.
```
```
* access() 메서드 이용하기 
...
지금까지 애플리케이션을 만들면서 이 두 가지를 분리해서 사용하는 경우는 보지 못함. 둘 중 하나를 사용하며
보통 Role(역할) 을 사용했다. 
```
```
* ROLE 을 사용해서 역할 정의하기
