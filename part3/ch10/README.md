...
CSRF 보호의 시작점은 필터 체인의 CsrfFiler 이다. 이 필터는 요청을 가로채고 GET,HEAD,TRACE,OPTIONS 외의 HTTP 방식을
포함한 모든 HTTP 방식의 요청을 허용하고 요청에 토큰이 포함되어 있는지 확인한다. (default 필터임)
...
```
토큰은 GET 요청시 프레임워크가 생성해서 응답 값으로 넘겨준다.

개발자의 관점에서는 CsrfFilter 요청 다음 토큰을 확인하는 로거를 만들고 GET 요청 후 
생성된 토큰을 로그로 확인하고 토큰값을 포함해서 POST 요청을 하면 된다! 

하지만 클라이언트는 서버에서 로그 값을 확인할 수 없다. 클라이언트가 사용할 HTTP 응답에
_csrf 토큰 값을 추가할 책임은 백엔드 애플리케이션에 있다.
```
