...
인증 프로세스가 끝난 후 인증된 엔티티의 세부 정보가 필요할 수 있다. 인증된 사용자의 이름이나 권한을 애플리케이션 로직에서 참조해야 될 때
...
스레드 로컬이란 해당 스레드만 접근할 수 있는 특별한 저장소를 의미한다 (스레드별 락커룸이라고 생각하자)
스프링 애플리케이션은 실행될 때 컴포넌트 스캔된 빈들이 각각 싱글톤 빈으로 구성되는데 여러 스레드에서 빈의 필드 값에

접근하면 스레드가 필드의 값이 공유된다. 하지만 스레드 로컬을 사용하면 각 스레드 요청 별로 정보를 저장할 수 있다.
...
### 비동기 호출을 위한 보유 전략 이용 MODE_INHERITABLETHREDLOCAL
```
대부분 앞서 설명한 기본 전략을 사용하지만 엔드포인트를 리액티브 스트림 즉 비동기로 만들면 요청의 각 단계별 
...
비동기 요청 시나리오를 구현하려면 @EnableAsync(설정 클래스) 와 @Async(별도의 엔드포인트) 애노테이션을 지정한다
이 애노테이션을 이용하면 요청을 비동기로 처리할 수 있다.
```
```
...
전략으로 MODE_INHERITABLETHREDLOCAL 을 사용한다.
...
```
```
* 인증 실패를 위한 응답의 영역 이름을 구성하기 138 p 
